...
import {
  KIMI_CODE_AGENT,
  KIMI_CODE_AGENT_CHANNELS,
  supportedDeepLinkSchemes,
} from "./kimi-brand"
...
const main = Effect.gen(function* () {
  ...
  const identity = app.isPackaged ? KIMI_CODE_AGENT_CHANNELS[CHANNEL] : KIMI_CODE_AGENT_CHANNELS.dev
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => supportedDeepLinkSchemes.some((scheme) => arg.startsWith(`${scheme}://`)))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient(KIMI_CODE_AGENT.scheme)
  registerRendererProtocol()
  setDockIcon()
