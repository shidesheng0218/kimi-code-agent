...
import { assertPublicReadOnlyTarget } from "./network-policy"
...
const assertPublicFetchTarget = (url: URL) =>
  assertPublicReadOnlyTarget(url, async (hostname) => lookup(hostname, { all: true, verbatim: true }))
          ...
          execute: (input, context) =>
            Effect.gen(function* () {
              yield* Effect.tryPromise({
                try: () => assertPublicFetchTarget(new URL(input.url)),
                catch: (error) => error,
              })
