...
enum ComputerUseController {
  static func diagnostics(promptForAccessibility: Bool = false) -> (isReady: Bool, message: String) {
  ...
  /// Adapter entry point used by the native Harness Tool Runtime. The
  /// coordinator has already completed capability and user approval checks
  /// before this method is called.
  static func executeHarnessRequest(_ request: ToolExecutionRequest) async throws -> ToolExecutionResult {
    ...
    case "computer_use.screenshot":
      guard CGPreflightScreenCaptureAccess() else {
        throw ComputerUseError.screenCapturePermissionRequired
      }
    ...
    case "computer_use.type_text":
      guard let text = request.input["text"], !text.isEmpty else {
        throw ComputerUseError.missingText
      }
      try type(text)
