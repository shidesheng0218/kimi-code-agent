...
// MARK: - Dynamic Planning Extension
extension AgentOrchestrator {
  /// Generate a dynamic execution plan using AI-powered task decomposition.
  /// This replaces the fixed 5-stage pipeline with context-aware subtask generation.
  public static func makeDynamicPlan(
    ...
    let bridge = DynamicPlanningBridge(vault: vault)
    ...
    // 转换为 AgentOrchestrationPlan
    var runs: [AgentRun] = []
    let subtaskIDToRunID = response.subtasks.reduce(into: [String: UUID]()) { dict, subtask in
      dict[subtask.id] = UUID()
    }

    for subtask in response.subtasks {
      guard let runID = subtaskIDToRunID[subtask.id] else { continue }
      let dependencies = subtask.dependencies.compactMap { subtaskIDToRunID[$0] }

      let agentKind: AgentKind
      switch subtask.agentKind {
      case "explore": agentKind = .explore
      case "implement": agentKind = .implement
      case "test": agentKind = .test
      case "review": agentKind = .review
      case "webResearch": agentKind = .webResearch
      case "browser": agentKind = .browser
      case "debug": agentKind = .debug
      default: agentKind = .explore
      }
      ...
      switch isolation {
      case .readOnlySnapshot: permissionMode = .readOnly
      case .sharedWorkspace, .worktree: permissionMode = .interactive
      }
    ...
    return AgentOrchestrationPlan(taskID: taskID, sessionID: sessionID, runs: runs)
  }
}
