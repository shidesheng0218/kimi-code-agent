...
@MainActor
public final class BrowserVerificationController: NSObject, WKNavigationDelegate, WKScriptMessageHandler {
  ...
  public func run(plan: BrowserVerificationPlan, artifactsDirectory: URL) async -> BrowserVerificationResult {
  ...
  public func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
    if let body = message.body as? [String: Any], let level = body["level"] as? String, level == "error" {
  ...
  private func perform(_ step: BrowserVerificationStep, artifactsDirectory: URL, timeline: inout [BrowserVerificationTrace], artifacts: inout [BrowserArtifact]) async throws {
    switch step.kind {
    case .open, .navigate:
    ...
    case .inspect:
      ...
      guard try await evaluateBooleanJavaScript("Boolean(document.querySelector(\(jsString(selector))))") else {
        throw BrowserVerificationError.selectorNotFound(selector)
      }
    ...
    case .pressKey:
      ...
      _ = try await evaluateBooleanJavaScript("document.activeElement?.dispatchEvent(new KeyboardEvent('keydown', { key: \(jsString(key)), bubbles: true }));")
    ...
    case .collectNetwork:
      let summary = try await evaluateStringJavaScript("JSON.stringify(performance.getEntriesByType('resource').map(entry => ({ name: entry.name, duration: entry.duration })))")
  ...
  private func captureScreenshot(name: String, directory: URL) async throws -> BrowserArtifact {
    let data: Data = try await withCheckedThrowingContinuation(isolation: MainActor.shared) { (continuation: CheckedContinuation<Data, Error>) in
      webView.takeSnapshot(with: nil) { image, error in
        if let error { continuation.resume(throwing: error) }
        else if let data = image?.pngData { continuation.resume(returning: data) }
        else { continuation.resume(throwing: BrowserVerificationError.screenshotFailed) }
      }
    }
